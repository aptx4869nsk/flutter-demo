...
enum PointsTransferType { cashIn, cashOut }

class PointsTransferHistoryLogic extends GetxController {
  final page = 1.obs;
  final pageSize = Get.find<IndexLogic>().pageSize;
  final controller = RefreshController();
  final selectedTransferType = PointsTransferType.cashIn.obs;
  ...
  PointsTransferType get transferType => selectedTransferType.value;

  // 转入/转出
  void toggleTransferType(PointsTransferType type) {
    ...
    LoadingView.singleton.wrap(asyncFunction: () async {
      try {
        TransactionHistories data = await Apis.getTransactionHistories(
          page: page.value,
          pageSize: pageSize,
          code: transferType == PointsTransferType.cashIn ? '001' : '002',
        );
